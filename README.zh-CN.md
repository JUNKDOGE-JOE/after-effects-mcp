...
```text
面板内对话或外部 MCP 客户端
  -> packages/core (ae_mcp, Python stdio MCP server, 49 个 ae_ 工具)
...
`ae_listCompositionLayers`，即可读取对应合成的有界图层分页（默认 25、最大 50）。原生通道
...
泄露原生 handle。原生通道不可用时这些工具会明确失败，不会回退到 JSX。
