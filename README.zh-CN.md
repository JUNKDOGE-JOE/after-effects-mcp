...
下一个候选版本是 **v0.9.1（未发布）**。当前只是同步候选构建所需的版本源，不代表签名产物或四格 AE 实机验收已经通过。

## v0.9.1 目标支持矩阵

未发布的 v0.9.1 候选只面向以下平台与宿主矩阵。在四格实机门禁通过前，下列矩阵不构成“已经支持”的发布声明：
...
MCP core 本身保持后端无关：外部客户端可以通过 stdio server 与 AE 对话，CEP 面板也可以在 AE 内承载内嵌 agent 对话。现有面板层负责后端配置、审批、诊断和活动历史。v0.9.1 最终契约还要求首跑包内 runtime 校验，但 RuntimeManager 行为仍受门禁，本文不声称它已经交付。Claude、Codex、ZCode 是面板内置后端；OpenCode 和其他工具仍可以作为外部 MCP 客户端接入。
...
- 正式发布仍受签名 helper/provider/Tool Library 的已批准实施里程碑、逐文件原生签名与产品验收覆盖、签名与再分发前置条件、Mac/Windows 上 AE 25/26 实机 smoke 及双 attestation 门禁约束。原生覆盖策略有意保持 fail-closed，单独落地 helper 或同步 v0.9.1 元数据都不能绕过这些门禁。
- UXP、Intel Mac、Windows ARM、provider 配置导出以及 ZCode 桌面 captcha/runtime-header 桥接不属于 v0.9.1 支持范围。

## 安装和首次启动

普通用户只安装 v0.9.1 发布组中的一个不可变平台资产。不要用源码归档或在线 `uv`/PyPI 安装替代签名发布资产：
...
macOS DMG 只封装精确的签名 ZXP，并为该容器提供签名与公证；DMG 本身不是 ZXP installer。两个平台都需要另行提供受支持的 ZXP installer。先用 `artifact-manifest-v0.9.1.json` 校验下载内容，再安装 DMG 内的 Mac ZXP 或 Windows ZXP、重启 After Effects，并打开 `Window -> Extensions -> ae-mcp`。在最终 gated 契约中，面板随后离线安装包内 runtime，校验成功后暴露稳定 launcher。

这些文件名定义的是 v0.9.1 契约。只有两个 attestation Check 都通过、no-rebuild 提升流程完成后，它们才会公开可用。详见[安装文档](docs/INSTALL.md)和[发布文档](docs/RELEASE.md)。
...
<table>
  ...
  <tr><td><img src="docs/images/zh-CN/settings-general-language.png" width="380"><br>设置页：通用选项、界面语言、日志与关于</td><td><img src="docs/images/zh-CN/wizard-install.png" width="380"><br>历史 v0.9.0 开发向导：在线 `uv` 与 PATH launcher 安装；不能代表 v0.9.1 包内 runtime UX</td></tr>
...
这是最终稳定 launcher 契约。请把 `<USER>` 替换为实际 macOS 账户名；最终 Panel 生成器必须输出展开后的绝对路径。Windows 必须使用 `%USERPROFILE%\.ae-mcp\bin\ae-mcp.exe` 展开后的绝对路径。RuntimeManager 获批实施后必须把当前 Panel 生成器的裸 PATH `ae-mcp` 改为平台绝对路径；fail-closed 原生/产品验收 build guard 会在该差异存在时阻止发布 v0.9.1。
...
开发部署前必须关闭全部 After Effects / AfterFX 进程。两个安装脚本都会先预检源码，在目标同一父目录完整 staging 并复验，原子保留旧面板为 backup，并打印绝对恢复命令。
...
维护者只能通过受保护的 `build-rc.yml` 工作流生成 v0.9.1 资产。Mac arm64 与 Windows x64 的精确字节由 `artifact-manifest-v0.9.1.json` 绑定，通过 `macos-rc-attestation` 与 `windows-rc-attestation` 后，再由 `release.yml` 原样提升且不重新构建。签名凭据、再分发批准、AE 25/26 安装和 Windows x64 验证机都是外部前置条件；完整流程见 [docs/RELEASE.md](docs/RELEASE.md)。
