// Contract tests for the Windows named-pipe endpoint registry.
//
// Scope note (deliberate, do not "fix"): the registry creates its pipe with a
// same-user ACL and performs NO peer authentication. Issue #88 (mutual
// transport authentication over the pipe) was closed NOT_PLANNED on
// 2026-07-28; the OS same-user boundary is the entire transport boundary on
// Windows. These tests pin the naming scheme, UUID validation, metadata
// bounds, stale-entry cleanup, and descriptor ownership — not admission.
...
struct TempRoot {
  TempRoot() {
    ...
    path = (std::filesystem::path(path)
            / ("aemcp-endpoint-test-" + std::to_string(::GetCurrentProcessId()))).string();
...
void publish_verify_and_cleanup() {
  TempRoot root;
  FakeBackend backend;
  WindowsEndpointRegistry registry(backend, EndpointRegistryConfig{
...
void stale_owned_endpoint_is_recovered() {
  ...
  const std::string stale_path =
      (std::filesystem::path(root.path) / "aemcp-n1"
       / "d-22222222-2222-4222-8222-222222222222.endpoint").string();
  std::filesystem::create_directories(
      std::filesystem::path(stale_path).parent_path());
  {
  ...
  WindowsEndpointRegistry fresh(backend, EndpointRegistryConfig{
...
void live_endpoint_is_never_collected() {
  ...
  const std::string live_path =
      (std::filesystem::path(root.path) / "aemcp-n1"
       / "d-33333333-3333-4333-8333-333333333333.endpoint").string();
  std::filesystem::create_directories(
      std::filesystem::path(live_path).parent_path());
  {
  ...
  WindowsEndpointRegistry fresh(backend, EndpointRegistryConfig{
      root.path, "eeeeffff0000", 2, 32});
  require(fresh.start(descriptor()).ok(), "fresh endpoint failed beside a live one");
  std::error_code error;
  require(std::filesystem::exists(live_path, error),
      "live descriptor was collected as stale");
}

void descriptor_parser_is_closed() {
  NativeEndpointDescriptor parsed;
  const std::string valid =
      WindowsEndpointRegistry::serialize_descriptor(descriptor());
  require(WindowsEndpointRegistry::parse_descriptor(valid, parsed),
  ...
  require(!WindowsEndpointRegistry::parse_descriptor(
