...
using PrimitiveExecutor = AdapterResult (*)(NativeProgramPrimitiveHost &,
                                            NativeHandleFrame &,
...
[[nodiscard]] AdapterResult adapter_failure(const Result &result) {
  ...
  failure.message =
      result.message.empty() ? "native primitive failed" : result.message;
...
[[nodiscard]] const JsonValue *member(const JsonObject &object,
...
[[nodiscard]] const JsonObject &required_object(const JsonObject &object,
                                                std::string_view name) {
  const JsonValue *value = member(object, name);
  const auto *nested =
      value == nullptr ? nullptr : std::get_if<JsonObject>(&value->value);
  if (nested == nullptr) {
    throw std::invalid_argument("native primitive argument is not an object: " +
                                std::string(name));
  }
  return *nested;
}

[[nodiscard]] const JsonValue::Array &required_array(const JsonObject &object,
                                                     std::string_view name) {
  const JsonValue *value = member(object, name);
  const auto *array =
      value == nullptr ? nullptr : std::get_if<JsonValue::Array>(&value->value);
  if (array == nullptr) {
    throw std::invalid_argument("native primitive argument is not an array: " +
                                std::string(name));
  }
  return *array;
}

[[nodiscard]] std::string required_string(const JsonObject &object,
                                          std::string_view name) {
  const JsonValue *value = member(object, name);
  const auto *text =
      value == nullptr ? nullptr : std::get_if<std::string>(&value->value);
  if (text == nullptr) {
    throw std::invalid_argument("native primitive argument is not a string: " +
                                std::string(name));
  }
  return *text;
}

[[nodiscard]] bool required_bool(const JsonObject &object,
                                 std::string_view name) {
  const JsonValue *value = member(object, name);
  const auto *boolean =
      value == nullptr ? nullptr : std::get_if<bool>(&value->value);
  if (boolean == nullptr) {
    throw std::invalid_argument("native primitive argument is not a boolean: " +
                                std::string(name));
  }
  return *boolean;
}
...
[[nodiscard]] Integer required_integer(const JsonObject &object,
                                       std::string_view name) {
  const JsonValue *value = member(object, name);
  const auto *number =
      value == nullptr ? nullptr : std::get_if<JsonNumber>(&value->value);
...
[[nodiscard]] JsonValue ratio_json(const CompositionPositiveRatio &ratio) {
  return json_value({
      ...
      {"denominator",
       json_number(static_cast<std::int64_t>(ratio.denominator))},
      {"rational", JsonValue{ratio.rational}},
  });
}

[[nodiscard]] CompositionCurrentTime
exact_time_from_json(const JsonObject &object) {
...
[[nodiscard]] LayerPropertySampleTime
sample_time_from_json(const JsonObject &object) {
  return {
...
[[nodiscard]] CompositionPositiveRatio
positive_ratio_from_json(const JsonObject &object) {
  ...
  return {
      numerator,
      denominator,
      canonical_seconds_rational(numerator,
                                 static_cast<std::uint32_t>(denominator)),
  };
}

[[nodiscard]] std::string
reference_name(const NativeProgramOperation &operation,
               std::string_view argument) {
  return required_string(required_object(operation.arguments, argument), "ref");
}

template <typename Handle>
[[nodiscard]] const Handle &
required_handle(const NativeProgramOperation &operation,
                NativeHandleFrame &frame, std::string_view argument,
                HandleKind kind) {
  return std::get<Handle>(
      frame.require(reference_name(operation, argument), kind));
}

[[nodiscard]] LayerPropertyValue
property_value_from_json(const JsonObject &object) {
  ...
  if (kind == "vector") {
    ...
    for (const JsonValue &value : required_array(object, "components")) {
      ...
      if (component == nullptr) {
        throw std::invalid_argument(
            "property vector component is not a string");
      }
...
[[nodiscard]] JsonValue project_items_json(const ProjectItemsPage &page) {
  ...
  JsonObject result{
      ...
      {"returned", json_number(static_cast<std::uint64_t>(page.items.size()))},
      {"hasMore", JsonValue{page.has_more}},
      {"items", json_array(std::move(items))},
  };
  result.emplace_back("nextOffset", page.next_offset.has_value()
                                        ? json_number(*page.next_offset)
                                        : JsonValue{nullptr});
  return json_value(std::move(result));
}

[[nodiscard]] JsonValue
composition_layers_json(const CompositionLayersPage &page) {
  ...
  for (const CompositionLayerEntry &layer : page.layers) {
    JsonObject value{
        ...
        {"parentLocator", layer.parent_locator.has_value()
                              ? locator_json(*layer.parent_locator)
                              : JsonValue{nullptr}},
        {"sourceItemLocator", layer.source_item_locator.has_value()
                                  ? locator_json(*layer.source_item_locator)
                                  : JsonValue{nullptr}},
    };
  ...
  JsonObject result{
      ...
      {"offset", json_number(page.offset)},
      {"limit", json_number(static_cast<std::uint64_t>(page.limit))},
      {"returned", json_number(static_cast<std::uint64_t>(page.layers.size()))},
      {"hasMore", JsonValue{page.has_more}},
...
[[nodiscard]] JsonValue
composition_time_json(const CompositionTimeRead &value) {
  return json_value({
...
[[nodiscard]] JsonValue
composition_time_changed_json(const CompositionTimeChanged &value) {
  return json_value({
...
[[nodiscard]] JsonValue
composition_settings_json(const CompositionSettings &value) {
  return json_value({
      {"compositionLocator", locator_json(value.composition_locator)},
      ...
      {"workArea", json_value({
                       {"start", exact_time_json(value.work_area_start)},
                       {"duration", exact_time_json(value.work_area_duration)},
                   })},
...
[[nodiscard]] JsonValue
composition_settings_changed_json(const CompositionSettingsChanged &value) {
  return json_value({
      {"changed", JsonValue{value.changed}},
      {"compositionLocator", locator_json(value.composition_locator)},
...
[[nodiscard]] JsonValue layer_properties_json(const LayerPropertiesPage &page) {
  ...
  for (const LayerPropertyEntry &property : page.properties) {
    ...
    value.emplace_back("canVaryOverTime",
                       property.can_vary_over_time.has_value()
                           ? JsonValue{*property.can_vary_over_time}
                           : JsonValue{nullptr});
    value.emplace_back("timeVarying", property.time_varying.has_value()
                                          ? JsonValue{*property.time_varying}
                                          : JsonValue{nullptr});
  ...
  JsonObject result{
      ...
      {"limit", json_number(static_cast<std::uint64_t>(page.limit))},
      {"returned",
       json_number(static_cast<std::uint64_t>(page.properties.size()))},
      {"hasMore", JsonValue{page.has_more}},
...
[[nodiscard]] JsonValue keyframes_json(const LayerPropertyKeyframesPage &page) {
  ...
  JsonObject result{
      ...
      {"limit", json_number(static_cast<std::uint64_t>(page.limit))},
      {"returned",
       json_number(static_cast<std::uint64_t>(page.keyframes.size()))},
      {"hasMore", JsonValue{page.has_more}},
...
[[nodiscard]] JsonValue
keyframe_ease_json(const LayerPropertyKeyframeDimensionEase &value) {
  return json_value({
...
[[nodiscard]] JsonValue
keyframe_details_json(const LayerPropertyKeyframeDetails &value) {
  ...
  return json_value({
      ...
      {"temporalDimensionality",
       json_number(static_cast<std::uint64_t>(value.temporal_dimensionality))},
      {"inInterpolation", JsonValue{value.in_interpolation}},
      {"outInterpolation", JsonValue{value.out_interpolation}},
      {"temporalEaseDimensions", json_array(std::move(ease))},
      {"behaviors",
       json_value({
           {"temporalContinuous",
           ...
           {"spatialContinuous", JsonValue{value.behavior.spatial_continuous}},
           {"spatialAutoBezier", JsonValue{value.behavior.spatial_auto_bezier}},
...
[[nodiscard]] JsonValue
property_changed_json(const LayerPropertyChanged &value) {
  return json_value({
      {"changed", JsonValue{value.changed}},
...
[[nodiscard]] JsonValue
keyframe_changed_json(const LayerPropertyKeyframeChanged &value) {
  return json_value({
      {"changed", JsonValue{value.changed}},
      {"layerLocator", locator_json(value.layer_locator)},
      {"propertyLocator", locator_json(value.property_locator)},
      ...
      {"beforeKeyframe", value.before.has_value()
                             ? keyframe_details_json(*value.before)
                             : JsonValue{nullptr}},
      {"afterKeyframe", value.after.has_value()
                            ? keyframe_details_json(*value.after)
                            : JsonValue{nullptr}},
  });
}

[[nodiscard]] AdapterResult execute_composition_resolve(
    NativeProgramPrimitiveHost &host, NativeHandleFrame &,
    const NativeProgramOperation &operation, TimePoint deadline) {
  ...
  return result.ok ? adapter_success(std::move(result.value))
                   : adapter_failure(result);
}

[[nodiscard]] AdapterResult execute_layer_resolve(
    NativeProgramPrimitiveHost &host, NativeHandleFrame &frame,
    const NativeProgramOperation &operation, TimePoint deadline) {
...
[[nodiscard]] AdapterResult execute_property_resolve(
  ...
  NativeHandleResolveResult result = host.resolve_native_handle(
      ...
      locator_from_json(required_object(operation.arguments, "locator")),
      layer.resolved.locator, deadline);
  return result.ok ? adapter_success(std::move(result.value))
                   : adapter_failure(result);
}
...
[[nodiscard]] AdapterResult execute_project_items_list(
  ...
  query.offset = required_integer<std::uint64_t>(operation.arguments, "offset");
  query.limit = required_integer<std::uint16_t>(operation.arguments, "limit");
  if (const JsonValue *locator =
          member(operation.arguments, "projectLocator")) {
  ...
  return result.ok ? adapter_success(project_items_json(result.value))
                   : adapter_failure(result);
}

[[nodiscard]] CompositionLayersQuery
composition_layers_query(NativeHandleFrame &frame,
...
[[nodiscard]] AdapterResult execute_composition_layers_list(
  ...
  return result.ok ? adapter_success(composition_layers_json(result.value))
                   : adapter_failure(result);
}
...
[[nodiscard]] AdapterResult execute_composition_selectedLayers_list(
    NativeProgramPrimitiveHost &host, NativeHandleFrame &frame,
    const NativeProgramOperation &operation, TimePoint deadline) {
  HostCompositionLayersResult result = host.list_selected_composition_layers(
      composition_layers_query(frame, operation), deadline);
  return result.ok ? adapter_success(composition_layers_json(result.value))
                   : adapter_failure(result);
}
...
[[nodiscard]] AdapterResult execute_composition_time_read(
  ...
  HostCompositionTimeResult result = host.read_composition_time(
      ...
      deadline);
  return result.ok ? adapter_success(composition_time_json(result.value))
                   : adapter_failure(result);
}
  ...
  HostCompositionTimeWriteResult result = host.set_composition_time(
      {
          ...
          composition.locator.session_id,
          composition.locator,
          exact_time_from_json(
              required_object(operation.arguments, "targetTime")),
      },
      deadline);
  return result.ok
...
[[nodiscard]] AdapterResult execute_composition_settings_read(
  ...
  HostCompositionSettingsResult result = host.read_composition_settings(
      ...
      deadline);
  return result.ok ? adapter_success(composition_settings_json(result.value))
                   : adapter_failure(result);
}
...
[[nodiscard]] AdapterResult execute_composition_setting_write(
    NativeProgramPrimitiveHost &host, NativeHandleFrame &frame,
    const NativeProgramOperation &operation, TimePoint deadline,
    CompositionSettingKind kind, std::string_view argument) {
  const auto &composition = required_handle<ScopedCompositionHandle>(
      operation, frame, "composition", HandleKind::kComposition);
  ...
  if (kind == CompositionSettingKind::kFrameRate ||
    ...
    command.time =
        exact_time_from_json(required_object(operation.arguments, argument));
  }
...
[[nodiscard]] AdapterResult execute_composition_duration_set(
    NativeProgramPrimitiveHost &host, NativeHandleFrame &frame,
    const NativeProgramOperation &operation, TimePoint deadline) {
  return execute_composition_setting_write(host, frame, operation, deadline,
...
[[nodiscard]] AdapterResult execute_layer_properties_list(
  ...
  return result.ok ? adapter_success(layer_properties_json(result.value))
                   : adapter_failure(result);
}
...
[[nodiscard]] AdapterResult execute_property_keyframes_list(
  ...
  HostLayerPropertyKeyframesResult result = host.list_layer_property_keyframes(
      {
      ...
      deadline);
  return result.ok ? adapter_success(keyframes_json(result.value))
                   : adapter_failure(result);
}
  ...
  HostLayerPropertyWriteResult result = host.set_layer_property(
      {
          ...
          layer.resolved.locator,
          property.locator,
          property_value_from_json(
              required_object(operation.arguments, "value")),
      },
      deadline);
  return result.ok ? adapter_success(property_changed_json(result.value))
                   : adapter_failure(result);
}
      ...
      host.read_layer_property_keyframe_details(
          {
              ...
              property.locator.session_id,
              property.locator,
              sample_time_from_json(
                  required_object(operation.arguments, "time")),
          },
          deadline);
  return result.ok ? adapter_success(keyframe_details_json(result.value))
                   : adapter_failure(result);
}

[[nodiscard]] LayerPropertyKeyframeMutationCommand
keyframe_command(NativeHandleFrame &frame,
                 const NativeProgramOperation &operation,
...
[[nodiscard]] AdapterResult
execute_keyframe_write(NativeProgramPrimitiveHost &host,
                       LayerPropertyKeyframeMutationCommand command,
                       TimePoint deadline) {
  HostLayerPropertyKeyframeWriteResult result =
      host.mutate_layer_property_keyframe(command, deadline);
  return result.ok ? adapter_success(keyframe_changed_json(result.value))
                   : adapter_failure(result);
}
...
[[nodiscard]] AdapterResult execute_property_keyframe_add(
    NativeProgramPrimitiveHost &host, NativeHandleFrame &frame,
    const NativeProgramOperation &operation, TimePoint deadline) {
  auto command = keyframe_command(frame, operation,
...
[[nodiscard]] AdapterResult execute_property_keyframe_value_set(
    ...
    const NativeProgramOperation &operation, TimePoint deadline) {
  auto command = keyframe_command(frame, operation,
                                  LayerPropertyKeyframeMutationKind::kSetValue);
  command.value =
      property_value_from_json(required_object(operation.arguments, "value"));
  return execute_keyframe_write(host, std::move(command), deadline);
}
...
[[nodiscard]] AdapterResult execute_property_keyframe_delete(
  ...
  return execute_keyframe_write(
      host,
      keyframe_command(frame, operation,
                       LayerPropertyKeyframeMutationKind::kDelete),
      deadline);
}
...
static_assert(kNativePrimitiveBindings.size() ==
              kNativePrimitiveRegistry.size());
...
NativeHandleFrame::NativeHandleFrame(std::string_view host_instance_id,
                                     std::string_view session_id)
    : host_instance_id_(host_instance_id), session_id_(session_id) {}

void NativeHandleFrame::save(std::string name, NativeHandle value) {
  if (name.empty() ||
      !handles_.emplace(std::move(name), std::move(value)).second) {
    throw std::invalid_argument("native handle name is empty or duplicated");
  }
}

const NativeHandle &NativeHandleFrame::require(std::string_view name,
...
void NativeHandleFrame::clear() noexcept { handles_.clear(); }

bool NativeHandleFrame::empty() const noexcept { return handles_.empty(); }
...
NativeHandleResolveResult
NativeHandleResolveResult::success(NativeHandle handle) {
...
NativeHandleResolveResult
NativeHandleResolveResult::failure(std::string code, std::string detail,
                                   std::string field) {
  NativeHandleResolveResult result;
...
NativeProgramHostResult
execute_native_program(NativeProgramPrimitiveHost &host,
  ...
  try {
    admission = validate_native_program(program, native_primitive_registry());
  } catch (const std::exception &error) {
    return NativeProgramHostResult::failure(
        "INVALID_ARGUMENT", error.what(), "params.arguments", {}, std::nullopt,
        false, NativeProgramDisposition::kNotStarted);
  }
  ...
  for (std::size_t index = 0; index < program.operations.size(); ++index) {
    if (std::chrono::steady_clock::now() >= work_deadline) {
      return NativeProgramHostResult::failure(
          completed_write ? "POSSIBLY_SIDE_EFFECTING_FAILURE"
                          : "DEADLINE_EXCEEDED",
          "native program execution budget elapsed", {}, std::move(completed),
          index, completed_write,
          completed_write ? NativeProgramDisposition::kPossiblySideEffecting
    ...
    if (binding_index < 0 ||
      ...
      return NativeProgramHostResult::failure(
          ...
          std::move(completed), index, completed_write,
          completed_write ? NativeProgramDisposition::kPossiblySideEffecting
                          : NativeProgramDisposition::kCompleted,
          std::move(outcomes), std::move(outputs));
    }
    ...
    try {
      result =
          kNativePrimitiveBindings[static_cast<std::size_t>(binding_index)]
              .execute(host, frame, program.operations[index], work_deadline);
    } catch (const std::exception &error) {
      const bool possibly_side_effecting =
          completed_write ||
          descriptor->mutability == PrimitiveMutability::kWrite;
      return NativeProgramHostResult::failure(
          possibly_side_effecting ? "POSSIBLY_SIDE_EFFECTING_FAILURE"
                                  : "CAPABILITY_FAILED",
          error.what(), {}, std::move(completed), index,
    ...
    if (!result.ok) {
      ...
      return NativeProgramHostResult::failure(
          possibly_side_effecting ? "POSSIBLY_SIDE_EFFECTING_FAILURE"
                                  : result.error_code,
          result.message, result.error_field, std::move(completed), index,
          possibly_side_effecting,
          possibly_side_effecting
              ? NativeProgramDisposition::kPossiblySideEffecting
              : NativeProgramDisposition::kCompleted,
    ...
    if (auto *handle = std::get_if<NativeHandle>(&result.value)) {
      ...
      if (program.operations[index].return_as.has_value()) {
        outputs.emplace_back(*program.operations[index].return_as,
                             public_value);
      }
    }
    outcomes.push_back({
  ...
  NativeProgramHostResult result =
      NativeProgramHostResult::success(std::move(outcomes), std::move(outputs));
