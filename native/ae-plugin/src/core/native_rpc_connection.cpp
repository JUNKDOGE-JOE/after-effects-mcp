...
rpc::ErrorResponse error_for(const ParsedRequest &request, const std::string &session_id,
                             std::string code, std::string message, std::string field = {},
                             std::string idempotency_key = {}) {
  RpcErrorCode mapped = rpc_error_code(code);
  if (request.method == RpcMethod::kHello && mapped != RpcErrorCode::kNativeUnavailable &&
      mapped != RpcErrorCode::kWireVersionMismatch && mapped != RpcErrorCode::kInvalidRequest &&
  ...
  if (request.method == RpcMethod::kInvoke &&
      (mapped == RpcErrorCode::kInvalidArgument || mapped == RpcErrorCode::kNativeUnsupported ||
       mapped == RpcErrorCode::kPreconditionFailed || mapped == RpcErrorCode::kStaleLocator ||
  ...
  if (mapped == RpcErrorCode::kPossiblySideEffectingFailure && !idempotency_key.empty()) {
    if (!response.details.has_value()) response.details = rpc::ErrorDetails{};
...
bool valid_timing_evidence(const ActiveEvidence &evidence, std::uint64_t completed_at_unix_ms) {
  return valid_digest(evidence.request_digest) && evidence.started_at_unix_ms >= 1 &&
...
bool collect_program_evidence(const ActiveEvidence &evidence, const NativeProgramHostResult &result,
                              bool completion_ok,
  ...
  if (result.failed_operation_index.has_value()) {
    ...
    if (index >= evidence.operations.size()) return false;
    failed = rpc::NativeProgramOperationSummary{index, evidence.operations[index], "failed"};
  }
  if (completion_ok) {
    return result.disposition == NativeProgramDisposition::kCompleted &&
           completed.size() == evidence.operations.size() && !failed.has_value();
  }
  return true;
}
...
std::string_view post_dispatch_evidence_failure_code(bool native_program_contains_write,
...
NativeRpcConnectionHandler::NativeRpcConnectionHandler(
    HostDispatcher &dispatcher, Clock &dispatcher_clock, rpc::SessionClock &session_clock,
    NativeRpcRuntimeInfo runtime, NativeRpcObserver &observer, HostIdleSignal &idle_signal)
...
void NativeRpcConnectionHandler::serve(const AuthenticatedConnection &connection) noexcept {
  try {
    rpc::RpcSessionFrontDoor front_door(connection.peer.connection_id, runtime_.host_instance_id,
    ...
    while (connected) {
      for (Completion &completion : dispatcher_.take_outbound()) {
        ...
        const bool graph_invalidation = completion.capability_id == kProjectGraphInvalidateControl;
        const bool native_program = completion.capability_id == kNativeProgramCapability;
        ...
        if (native_program && evidence_valid) {
          try {
            evidence_valid =
                collect_program_evidence(evidence->second, completion.native_program_result,
                                         completion.ok, completed_operations, failed_operation);
            if (evidence_valid) {
              postcondition_digest = rpc::digest_native_program_postcondition(
                  completion.native_program_result.outputs, completed_operations);
            }
          } catch (...) {
            evidence_valid = false;
          }
        } else if (graph_invalidation && evidence_valid && completion.ok) {
          const ProjectGraphInvalidation &result = completion.project_graph_invalidation_result;
          evidence_valid = result.invalidated
                               ? result.generation >= 1 && result.generation <= rpc::kMaxSafeInteger
                               : result.generation == 0;
        }
        ...
        if (!evidence_valid) {
          ...
          completion.error_code = post_dispatch_evidence_failure_code(
              native_program && evidence->second.contains_write, graph_invalidation);
          ...
          if (native_program) {
            ...
            result.disposition = evidence->second.contains_write
          ...
          observer_.on_rpc_event("terminal.validation", completion.request_id, "invalid-evidence");
        }

        observer_.on_rpc_terminal(
            completion,
            valid_digest(request_digest) ? std::string_view(request_digest) : std::string_view{},
        ...
        if (native_program && completion.ok) {
          const NativeProgramHostResult &result = completion.native_program_result;
          response = rpc::encode_native_program_success({
              ...
              result.undo_available ? std::optional<std::string>{evidence->second.undo_group}
                                    : std::nullopt,
              completion.replayed,
          });
          ...
          response = rpc::encode_native_program_failure({
              ...
              completion.message.empty() ? "native program failed" : completion.message,
              result.disposition,
              completed_operations,
              failed_operation,
              result.outputs,
          ...
          response = rpc::encode_error_response(
              error_for(synthetic, connection.session_id,
      ...
      const int received = transport_recv(connection.socket_fd, input.data(), input.size(), 20);
      if (received == 0) break;
      if (received < 0) {
      ...
      for (ParsedRequest &request : decoder.push(
        ...
        if (!ingress.accepted()) {
          const std::string message =
              request.malformed ? (request.malformed_error.empty() ? "native request was rejected"
          ...
          if (!write_frame(connection.socket_fd,
                           rpc::encode_error_response(error_for(request, connection.session_id,
        ...
        if (request.method == RpcMethod::kCapabilities) {
          ...
          const bool selected =
              !query.ids.has_value() || std::find(query.ids->begin(), query.ids->end(),
                                                  kNativeProgramCapability) != query.ids->end();
          if (selected && query.limit < 1) {
            if (!write_frame(connection.socket_fd,
                             rpc::encode_error_response(
                                 error_for(request, connection.session_id, "INVALID_ARGUMENT",
          ...
          if (selected) selected_indices.push_back(0);
          if (!write_frame(connection.socket_fd,
        ...
        if (request.method == RpcMethod::kCancel) {
          ...
          const CancelResult result =
              dispatcher_.cancel(connection.peer.connection_id, connection.session_generation,
                                 cancel.target_request_id);
          if (result.code == CancelCode::kInvalidRequest ||
              result.code == CancelCode::kStaleRoute) {
            const std::string code =
                result.code == CancelCode::kStaleRoute ? "SESSION_STALE" : "INVALID_ARGUMENT";
            if (!write_frame(connection.socket_fd, rpc::encode_error_response(error_for(
                                                       request, connection.session_id, code,
        ...
        const std::uint64_t ttl = effective_deadline > now_unix ? effective_deadline - now_unix : 0;
        Request dispatch_request;
        if (request.method == RpcMethod::kInvalidateGraph) {
          dispatch_request.request_id = request.request_id;
          dispatch_request.capability_id = std::string(kProjectGraphInvalidateControl);
          dispatch_request.deadline = dispatcher_clock_.now() + std::chrono::milliseconds(ttl);
          dispatch_request.route_id = connection.peer.connection_id;
          dispatch_request.session_generation = connection.session_generation;
        } else {
          const auto &native = std::get<rpc::NativeProgramParams>(request.params);
          dispatch_request.request_id = request.request_id;
        ...
        if (enqueued.code != EnqueueCode::kAccepted) {
          if (!write_frame(connection.socket_fd,
                           rpc::encode_error_response(error_for(
                               ...
                               enqueued.message.empty() ? "native dispatcher rejected the request"
                                                        : enqueued.message,
        ...
        if (const auto *native = std::get_if<rpc::NativeProgramParams>(&request.params)) {
          ...
          for (const NativeProgramOperation &operation : native->program.operations) {
  ...
  try {
    (void)dispatcher_.revoke_route(connection.peer.connection_id, connection.session_generation);
  } catch (...) {
