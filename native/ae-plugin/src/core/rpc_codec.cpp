...
std::vector<std::uint8_t> encode_legacy_capabilities_success(const CapabilitiesSuccess& response) {
  ...
  require_digest(response.capabilities_digest, "capabilities digest");
  if (response.capabilities_digest != kCapabilitiesRegistryDigest) {
    invalid_argument("capabilities digest does not match the compiled registry");
  }
  const std::string detail = response.detail == CapabilityDetail::kFull ? "full" : "summary";
  std::string items = "[";
