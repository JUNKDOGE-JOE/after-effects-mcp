...
std::string required_string(const JsonObject &object, std::string_view name) {
  ...
  if (result == nullptr || result->empty())
    invalid("missing or invalid " + std::string(name));
  return *result;
}
...
std::string sha256_hex(std::string_view input) {
  ...
  for (const std::uint32_t word : state)
    for (int shift = 28; shift >= 0; shift -= 4)
      output.push_back(hex[(word >> shift) & 0xfU]);
  return output;
}
...
private:
  ...
  void literal(std::string_view literal_value) {
    if (text_.substr(offset_, literal_value.size()) != literal_value)
  ...
  std::string string() {
    ...
    while (offset_ < text_.size()) {
      ...
      if (character == '\\') {
        ...
        case 'u': {
          ...
          const auto unit = [&]() -> std::uint32_t {
            ...
            for (int index = 0; index != 4; ++index)
              code = (code << 4U) | hex(text_[offset_++]);
            return code;
          };
        ...
        default:
          invalid("unsupported string escape");
        }
      } else {
...
const NativePrimitiveDescriptor *
lookup(std::string_view id,
...
bool validates_literal(const JsonValue &value, const JsonObject &schema) {
  // Generated admission schemas inline every local root definition.  Rejecting
  // a remaining ref is intentional: it would otherwise bypass pre-dispatch
  // checks.
  if (member(schema, "$ref") != nullptr)
    return false;
  for (const std::string_view composition : {"allOf", "anyOf", "oneOf"}) {
    ...
    for (const JsonValue &branch : *branches) {
      ...
      if (branch_schema != nullptr && validates_literal(value, *branch_schema))
        ++matches;
    }
    if ((composition == "allOf" && matches != branches->size()) ||
        (composition == "anyOf" && matches == 0) ||
        (composition == "oneOf" && matches != 1))
      return false;
  }
  if (const JsonValue *constant = member(schema, "const"))
    return canonical_json(value) == canonical_json(*constant);
  ...
  const std::string *type = member(schema, "type") == nullptr
                                ? nullptr
                                : string_of(*member(schema, "type"));
  const bool object_rules = (type != nullptr && *type == "object") ||
                            member(schema, "properties") != nullptr;
  if (object_rules) {
    const auto *object = object_of(value);
    if (object == nullptr)
      return false;
    const auto *properties = member(schema, "properties") == nullptr
                                 ? nullptr
                                 : object_of(*member(schema, "properties"));
    const auto *required = member(schema, "required") == nullptr
                               ? nullptr
                               : array_of(*member(schema, "required"));
    if (properties == nullptr && required != nullptr)
      return false;
    for (const auto &item : *object) {
      const JsonValue *child =
          properties == nullptr ? nullptr : member(*properties, item.first);
      if (child == nullptr) {
        const JsonValue *closed = member(schema, "additionalProperties");
        if (closed != nullptr && std::holds_alternative<bool>(closed->value) &&
            !std::get<bool>(closed->value))
          return false;
        continue;
      }
      const auto *child_schema = object_of(*child);
      if (child_schema == nullptr ||
          !validates_literal(item.second, *child_schema))
        return false;
    }
    if (required != nullptr)
      for (const JsonValue &required_name : *required) {
        const auto *name = string_of(required_name);
        if (name == nullptr || member(*object, *name) == nullptr)
          return false;
      }
    return true;
  }
  ...
  if (*type == "array") {
    ...
    const auto *items = member(schema, "items") == nullptr
                            ? nullptr
                            : object_of(*member(schema, "items"));
    if (items == nullptr)
      return false;
    for (const JsonValue &item : *array)
      if (!validates_literal(item, *items))
        return false;
    return true;
  }
  if (*type == "string") {
    const auto *string = string_of(value);
    if (string == nullptr)
      return false;
    if (const auto *minimum = member(schema, "minLength"); minimum != nullptr) {
      const auto *number = number_of(*minimum);
      if (number == nullptr ||
          string->size() < static_cast<std::size_t>(number->value))
        return false;
    }
    ...
    if (const auto *pattern = member(schema, "pattern"); pattern != nullptr) {
      ...
      if (expression == nullptr ||
          !std::regex_match(*string, std::regex(*expression)))
        return false;
    }
    return true;
  }
...
JsonValue parse_json(std::string_view json) { return Parser(json).parse(); }
...
std::string canonicalize_json(const JsonValue &value) {
  return canonical_json(value);
}

std::string sha256_hex_digest(std::string_view value) {
  return sha256_hex(value);
}

NativeProgram parse_native_program(const JsonObject &object) {
  if (!exact_keys(object, {"operationKey", "undoGroup", "operations"},
                  {"operations"})) {
  ...
  if (member(object, "operationKey") != nullptr) {
    ...
    if (!idempotency_key_shape(result.operation_key))
      invalid("operationKey does not match wire schema");
  }
  if (member(object, "undoGroup") != nullptr) {
    result.undo_group = required_string(object, "undoGroup");
    if (result.undo_group.size() > 128)
      invalid("undoGroup does not match wire schema");
  }
  const JsonValue *operations_value = member(object, "operations");
  const auto *operations =
      operations_value == nullptr ? nullptr : array_of(*operations_value);
  if (operations == nullptr || operations->empty() ||
      operations->size() > kMaxNativeProgramOperations) {
  ...
  for (const JsonValue &operation_value : *operations) {
    ...
    if (operation == nullptr ||
        !exact_keys(*operation, {"op", "args", "saveAs", "returnAs"},
                    {"op", "args"})) {
    ...
    if (arguments == nullptr)
      invalid("operation args must be an object");
    ...
    if (member(*operation, "saveAs") != nullptr) {
      ...
      if (parsed.save_as->size() > 64)
        invalid("saveAs does not match wire schema");
    }
    if (member(*operation, "returnAs") != nullptr) {
      parsed.return_as = required_string(*operation, "returnAs");
      if (parsed.return_as->size() > 64)
        invalid("returnAs does not match wire schema");
    }
...
validate_native_program(const NativeProgram &program,
  ...
  if (program.operations.empty() ||
      program.operations.size() > kMaxNativeProgramOperations) {
    invalid("operations must contain 1..64 items");
  }
  ...
  for (const NativeProgramOperation &operation : program.operations) {
    const NativePrimitiveDescriptor *descriptor =
        lookup(operation.primitive_id, registry);
    if (descriptor == nullptr)
      invalid("unknown primitive " + operation.primitive_id);
    result.descriptors.push_back(descriptor);
    result.contains_write =
        result.contains_write ||
        descriptor->mutability == PrimitiveMutability::kWrite;
    const JsonObject literal_schema =
        parse_json_object(descriptor->input_schema_json);
    const JsonValue *properties_value = member(literal_schema, "properties");
    const JsonObject *literal_properties =
        properties_value == nullptr ? nullptr : object_of(*properties_value);
    const JsonValue *required_value = member(literal_schema, "required");
    const JsonValue::Array *literal_required =
        required_value == nullptr ? nullptr : array_of(*required_value);
    if (literal_properties == nullptr || literal_required == nullptr)
      invalid("generated literal schema is invalid");
    std::set<std::string_view> declared_refs;
    for (const NativeReferenceArgument &reference :
         descriptor->reference_arguments) {
      declared_refs.insert(reference.name);
      const JsonValue *candidate = member(operation.arguments, reference.name);
      if (candidate == nullptr && !reference.required)
        continue;
      const JsonObject *ref =
          candidate == nullptr ? nullptr : object_of(*candidate);
      if (ref == nullptr || !exact_keys(*ref, {"ref"}, {"ref"})) {
        invalid("missing or invalid reference argument " +
      ...
      if (named == result.named_value_kinds.end())
        invalid("forward or unknown reference " + name);
      if (named->second != reference.kind)
        invalid("reference kind mismatch for " + std::string(reference.name));
    }
    for (const auto &argument : operation.arguments) {
      if (declared_refs.contains(argument.first))
        continue;
      if (member(*literal_properties, argument.first) == nullptr) {
        invalid("unexpected literal argument " + argument.first);
      }
      const auto *argument_schema =
          object_of(*member(*literal_properties, argument.first));
      if (argument_schema == nullptr ||
          !validates_literal(argument.second, *argument_schema)) {
        invalid("invalid literal argument " + argument.first);
      }
      if (contains_ref(argument.second))
        invalid("reference used outside generated reference arguments");
    }
    ...
    if (operation.save_as.has_value()) {
      if (!names.insert(*operation.save_as).second)
        invalid("duplicate saveAs " + *operation.save_as);
      result.named_value_kinds.emplace(*operation.save_as,
...
std::string canonical_native_program(const NativeProgram &program) {
  ...
  for (std::size_t index = 0; index < program.operations.size(); ++index) {
    ...
    canonical += "{\"args\":" + canonical_json(JsonValue{operation.arguments}) +
                 ",\"op\":";
