...
|---|---|---|
| `GET /v1/models` | `GET <base>/v1/models` | body 不存在；保留 query；status/body 透明传递但响应头经过 allowlist |
| `POST /v1/responses` | `POST <base>/v1/chat/completions` | 只转换已声明支持的 Responses 字段、function tools 与对应 SSE/JSON 输出；未知或不可表达字段返回 400，不静默丢弃 |
...
- 优先使用无副作用的 capability/model 探测；只有命中已知成功 schema 或特定 endpoint 语义才记录 dialect。
- 任意 HTTP 400 JSON、HTML 错误页、WAF challenge 或登录重定向都不能视为支持。
- 探测结果带 base URL、auth profile revision 和时间戳；任一项改变即失效。
