...
v0.9.5 是 Windows x64 修复版本。ZXP 内含 CEP 面板与 Windows Platform Helper；原生 AEX 作为独立签名资产手动安装。外部 runtime/launcher 可以由面板首跑向导联网安装；本版不内置离线 Python，也不是只装 ZXP 即用的安装包。
...
1. 下载 ZXP、AEX 与 `SHA256SUMS-v0.9.5.txt`，先校验两个二进制。
...
4. 重启 AE 并打开面板。若缺少 `uv` 或 `ae-mcp`，使用首跑向导联网安装；Windows 向导通过 winget（失败时使用 uv 官方 PowerShell 安装脚本）安装 `uv`，再执行固定到 `v0.9.5` tag 的 `uv tool install --force --from git+https://github.com/JUNKDOGE-JOE/after-effects-mcp@v0.9.5#subdirectory=packages/core ...`。已有兼容 runtime/launcher 会被复用。
...
Windows v0.9.5 继续使用 ZXP 外部的 runtime/launcher，但首跑向导会在缺失时联网安装它；ZXP 本身不携带 Python runtime。
...
- launcher 不存在：重新运行面板首跑向导，联网安装 `uv` 与固定到 v0.9.5 tag 的 `ae-mcp` runtime；若自定义了 `UV_TOOL_BIN_DIR`，复制面板生成的实际路径。
...
### v0.9.5 Status and Support Matrix
...
1. Download the ZXP, AEX, and `SHA256SUMS-v0.9.5.txt`; verify both binaries first.
...
4. Restart AE and open the Panel. If `uv` or `ae-mcp` is missing, use the first-run wizard. On Windows it installs `uv` through winget (falling back to uv's official PowerShell installer), then runs a tag-pinned `uv tool install --force --from git+https://github.com/JUNKDOGE-JOE/after-effects-mcp@v0.9.5#subdirectory=packages/core ...`. An existing compatible runtime/launcher is reused.
...
Windows v0.9.5 continues to use a runtime/launcher outside the ZXP, but the first-run wizard installs it online when missing. The ZXP itself does not carry a Python runtime.
...
- Launcher missing: rerun the Panel's first-run wizard to install `uv` and the v0.9.5-tagged `ae-mcp` runtime online. If `UV_TOOL_BIN_DIR` is customized, copy the actual path generated by the Panel.
