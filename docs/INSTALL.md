...
v0.9.1 当前是未发布候选契约。仓库里的版本号与文件名用于构建候选，并不代表签名、公证、helper、请求头路由、Tool Library 或四格 AE 实机验收已经完成。只有 [发布门禁](RELEASE.md)全部通过后，下面的资产才会公开。
...
两个平台的精确文件都由 `artifact-manifest-v0.9.1.json` 绑定到同一个 protected `main` candidate SHA。不要用源码归档、本地重建包、公共 PyPI 同名包或在线依赖安装替代它们。

macOS DMG 只封装 manifest 绑定的签名 ZXP，不包含也不冒充第三方安装器。受支持的 ZXP installer 必须为两个平台另行提供；同一个安装器契约也用于 Mac/Windows RC verifier。

1. 从同一个 GitHub Release 下载当前平台资产和 `artifact-manifest-v0.9.1.json`。
...
上例是 v0.9.1 最终稳定 launcher 契约。当前 Panel 配置生成器仍输出裸 PATH `ae-mcp`；RuntimeManager 未获明确批准和实现前不得偷偷改写该生成器，原生/产品验收 build guard 会在这项差异未闭环时阻止正式候选。
...
### v0.9.1 Status and Support Matrix

v0.9.1 is currently an unreleased candidate contract. Repository versions and filenames prepare a candidate; they do not assert that signing, notarization, the platform helper, provider routing, Tool Library, or the four-cell AE hardware matrix is complete. The assets below become public only after every [release gate](RELEASE.md) passes.
...
The v0.9.1 platform assets are:
...
`artifact-manifest-v0.9.1.json` binds the exact files for both platforms to one protected `main` candidate SHA. Do not substitute a source archive, locally rebuilt package, public PyPI namesake, or online dependency install.
...
1. Download the platform asset and `artifact-manifest-v0.9.1.json` from the same GitHub Release.
...
This is the final v0.9.1 stable-launcher contract. The current Panel config generator still emits bare PATH `ae-mcp`; it must not be silently changed before explicit RuntimeManager approval and implementation, and the native/product-acceptance build guard blocks a formal candidate while this gap remains.
