# v0.9.2 不可变双平台 RC / Immutable Dual-Platform RC

## 中文

### 1. 状态与原则

v0.9.2 当前是**未发布候选**。版本源同步、workflow 文件存在或单个平台测试通过，都不等于发布完成。正式 tag 只能指向 protected `main` 上已验证的 candidate SHA，并且必须原样提升已经通过实机验证的字节：**禁止重建**。
...
v0.9.2 is an **unreleased candidate**. Synchronized versions, workflow files, or one passing platform do not constitute a release. The final tag may point only to a verified protected-`main` candidate SHA, and promotion must reuse the exact hardware-tested bytes: **no rebuild**.
