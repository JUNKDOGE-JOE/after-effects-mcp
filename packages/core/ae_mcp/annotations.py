"""Single source of truth for MCP tool annotations.

External agent frameworks run their own approval tiers; these hints are what
their auto-review modes branch on. Keep the dict exhaustive - test_annotations
enforces one entry per registered verb.
"""
from __future__ import annotations

from mcp.types import ToolAnnotations


def _ann(read_only: bool, destructive: bool, idempotent: bool) -> ToolAnnotations:
    return ToolAnnotations(
        readOnlyHint=read_only,
        destructiveHint=destructive,
        idempotentHint=idempotent,
    )


# Classification rules:
# - read/diagnostic/list/get/status/search/snapshot/preview/validate -> read-only
# - set/move/select/update-style writes -> non-destructive, usually idempotent
# - create/apply/checkpoint/use-style writes -> non-destructive, not idempotent
# - delete/revert/arbitrary code execution -> destructive, not idempotent
VERB_ANNOTATIONS: dict[str, ToolAnnotations] = {
    "ae.init": _ann(True, False, True),
    "ae.overview": _ann(True, False, True),
    "ae.projectSummary": _ann(True, False, True),
    "ae.getProjectContext": _ann(True, False, True),
    "ae.getProjectItemMetadata": _ann(True, False, True),
    "ae.getCompositionSettings": _ann(True, False, True),
    "ae.setCompositionWorkArea": _ann(False, False, True),
    "ae.setCompositionDimensions": _ann(False, True, True),
    "ae.setCompositionDuration": _ann(False, True, True),
    "ae.setCompositionFrameRate": _ann(False, True, True),
    "ae.setCompositionPixelAspectRatio": _ann(False, True, True),
    "ae.setCompositionBackgroundColor": _ann(False, True, True),
    "ae.setCompositionDisplayStartTime": _ann(False, True, True),
    "ae.renameProjectItem": _ann(False, False, True),
    "ae.setProjectItemComment": _ann(False, False, True),
    "ae.setProjectItemLabel": _ann(False, False, True),
    "ae.duplicateComposition": _ann(False, False, True),
    "ae.getProjectBitDepth": _ann(True, False, True),
    "ae.setProjectBitDepth": _ann(False, False, True),
    "ae.listProjectItems": _ann(True, False, True),
    "ae.listCompositionLayers": _ann(True, False, True),
    "ae.listSelectedLayers": _ann(True, False, True),
    "ae.getLayerDetails": _ann(True, False, True),
    "ae.getLayerCompositingState": _ann(True, False, True),
    "ae.setLayerVisibility": _ann(False, False, True),
    "ae.setLayerSolo": _ann(False, False, True),
    "ae.setLayerLocked": _ann(False, False, True),
    "ae.setLayerShy": _ann(False, False, True),
    "ae.setLayerMotionBlur": _ann(False, False, True),
    "ae.setLayerThreeD": _ann(False, False, True),
    "ae.setLayerAdjustment": _ann(False, False, True),
    "ae.setLayerQuality": _ann(False, False, True),
    "ae.setLayerBlendingMode": _ann(False, False, True),
    "ae.getLayerSource": _ann(True, False, True),
    "ae.setLayerSource": _ann(False, False, True),
    "ae.getLayerTrackMatte": _ann(True, False, True),
    "ae.setLayerTrackMatte": _ann(False, False, True),
    "ae.clearLayerTrackMatte": _ann(False, False, True),
    "ae.getLayerAVState": _ann(True, False, True),
    "ae.setLayerAudioEnabled": _ann(False, False, True),
    "ae.setLayerVideoEnabled": _ann(False, False, True),
    "ae.getLayerTransform": _ann(True, False, True),
    "ae.setLayerAnchorPoint": _ann(False, False, True),
    "ae.setLayerPosition": _ann(False, False, True),
    "ae.setLayerScale": _ann(False, False, True),
    "ae.setLayerRotation": _ann(False, False, True),
    "ae.setLayerOpacity": _ann(False, False, True),
    "ae.setLayerOrientation": _ann(False, False, True),
    "ae.renameLayer": _ann(False, False, True),
    "ae.setLayerRange": _ann(False, False, True),
    "ae.setLayerStartTime": _ann(False, False, True),
    "ae.setLayerStretch": _ann(False, False, True),
    "ae.reorderLayer": _ann(False, False, True),
    "ae.setLayerParent": _ann(False, False, True),
    "ae.duplicateLayer": _ann(False, False, True),
    "ae.getCompositionTime": _ann(True, False, True),
    "ae.setCompositionTime": _ann(False, False, True),
    "ae.createComposition": _ann(False, False, True),
    "ae.createCompositionLayer": _ann(False, False, True),
    "ae.applyLayerEffect": _ann(False, False, True),
    "ae.listInstalledEffects": _ann(True, False, True),
    "ae.listLayerEffects": _ann(True, False, True),
    "ae.getLayerEffectDetails": _ann(True, False, True),
    "ae.setLayerEffectEnabled": _ann(False, False, True),
    "ae.reorderLayerEffect": _ann(False, False, True),
    "ae.duplicateLayerEffect": _ann(False, False, True),
    "ae.deleteLayerEffect": _ann(False, True, True),
    "ae.listLayerMasks": _ann(True, False, True),
    "ae.getLayerMaskDetails": _ann(True, False, True),
    "ae.getLayerMaskPath": _ann(True, False, True),
    "ae.createLayerMask": _ann(False, False, True),
    "ae.setLayerMaskProperties": _ann(False, False, True),
    "ae.setLayerMaskPath": _ann(False, False, True),
    "ae.duplicateLayerMask": _ann(False, False, True),
    "ae.deleteLayerMask": _ann(False, True, True),
    "ae.getFootageDetails": _ann(True, False, True),
    "ae.importFootage": _ann(False, False, True),
    "ae.replaceFootage": _ann(False, False, True),
    "ae.getFootageInterpretation": _ann(True, False, True),
    "ae.setFootageInterpretation": _ann(False, False, True),
    "ae.setFootageProxy": _ann(False, False, True),
    "ae.setItemUseProxy": _ann(False, False, True),
    # Closed structured requests rendered by maintained repository templates.
    # None accepts code, JSX, expressions, match names, or property paths.
    "ae.listInstalledFonts": _ann(True, False, True),
    "ae.createTextLayer": _ann(False, False, True),
    "ae.getTextDocument": _ann(True, False, True),
    "ae.setTextContent": _ann(False, False, True),
    "ae.setTextCharacterStyle": _ann(False, False, True),
    "ae.setTextParagraphStyle": _ann(False, False, True),
    # Bounded native operations. Stable business keys make writes idempotent;
    # deleteMarker is destructive because it removes authored state.
    "ae.createShapeLayer": _ann(False, False, True),
    "ae.listShapeGroups": _ann(True, False, True),
    "ae.createShapeGroup": _ann(False, False, True),
    "ae.setShapePath": _ann(False, False, True),
    "ae.setShapeFillStyle": _ann(False, False, True),
    "ae.setShapeStrokeStyle": _ann(False, False, True),
    "ae.reorderShapeGroup": _ann(False, False, True),
    "ae.listMarkers": _ann(True, False, True),
    "ae.createMarker": _ann(False, False, True),
    "ae.setMarker": _ann(False, False, True),
    "ae.deleteMarker": _ann(False, True, True),
    "ae.listLayerProperties": _ann(True, False, True),
    "ae.listLayerPropertyKeyframes": _ann(True, False, True),
    "ae.setLayerPropertyValue": _ann(False, False, True),
    "ae.getLayerPropertyKeyframeDetails": _ann(True, False, True),
    "ae.addLayerPropertyKeyframe": _ann(False, False, True),
    "ae.setLayerPropertyKeyframeValue": _ann(False, False, True),
    "ae.setLayerPropertyKeyframeInterpolation": _ann(False, False, True),
    "ae.setLayerPropertyKeyframeTemporalEase": _ann(False, False, True),
    "ae.setLayerPropertyKeyframeBehavior": _ann(False, False, True),
    "ae.deleteLayerPropertyKeyframe": _ann(False, True, True),
    "ae.layers": _ann(True, False, True),
    "ae.readProps": _ann(False, True, False),
    "ae.exec": _ann(False, True, False),
    # Bounded programs may include native writes. A stable operationKey binds
    # every mutating program to one terminal result and prevents redispatch.
    "ae.nativeExec": _ann(False, True, True),
    "ae.checkpoint": _ann(False, False, False),
    "ae.revert": _ann(False, True, False),
    "ae.snapshot": _ann(True, False, True),
    "ae.previewFrame": _ann(True, False, True),
    "ae.applyEffect": _ann(False, False, False),
    "ae.ping": _ann(True, False, True),
    "ae.createRig": _ann(False, False, False),
    "ae.skillList": _ann(True, False, True),
    "ae.skillCreate": _ann(False, False, False),
    "ae.skillEdit": _ann(False, False, True),
    "ae.skillDelete": _ann(False, True, False),
    # skillUse can execute stored arbitrary JSX (execute=true) — ae.exec by
    # proxy, so it carries the same worst-path destructive hint.
    "ae.skillUse": _ann(False, True, False),
    "ae.toolIndex": _ann(True, False, True),
    "ae.toolSearch": _ann(True, False, True),
    "ae.toolInspect": _ann(True, False, True),
    "ae.toolUse": _ann(False, True, False),
    "ae.toolCreate": _ann(False, False, False),
    "ae.toolEdit": _ann(False, False, True),
    "ae.toolDelete": _ann(False, True, False),
    "ae.toolArchive": _ann(False, False, True),
    "ae.toolDuplicate": _ann(False, False, False),
    "ae.toolPromoteFromHistory": _ann(False, False, True),
    "ae.toolImport": _ann(False, False, False),
    "ae.toolExport": _ann(False, True, True),
    "ae.status": _ann(True, False, True),
    "ae.diagnose": _ann(True, False, True),
    "ae.createLayer": _ann(False, False, False),
    "ae.setProperty": _ann(False, True, False),
    "ae.moveLayer": _ann(False, False, True),
    "ae.selectLayers": _ann(False, False, True),
    "ae.setTime": _ann(False, False, True),
    "ae.getTime": _ann(True, False, True),
    "ae.getProperties": _ann(True, False, True),
    "ae.scanPropertyTree": _ann(True, False, True),
    "ae.inspectPropertyCapabilities": _ann(True, False, True),
    "ae.getExpressions": _ann(True, False, True),
    "ae.validateExpressions": _ann(True, False, True),
    "ae.getKeyframes": _ann(True, False, True),
    "ae.searchProject": _ann(True, False, True),
}
