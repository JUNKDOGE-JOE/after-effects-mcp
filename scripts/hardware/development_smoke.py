...
PROCESS_START_TOKEN = re.compile(
...
class FormalAEProcessReceipt:
    ...
    def __post_init__(self) -> None:
        require(
            Path(self.formal_ae_app).is_absolute()
            and Path(self.executable_path).is_absolute(),
            "formal AE process receipt paths must be absolute",
        )
        require(
            Path(self.executable_path).is_relative_to(
                Path(self.formal_ae_app) / "Contents/MacOS"
            ),
            "formal AE process receipt executable escaped its application",
        )
        require(
            isinstance(self.pid, int)
            and not isinstance(self.pid, bool)
            and self.pid > 0,
            "formal AE process receipt PID is invalid",
        )
        require(
            bool(PROCESS_START_TOKEN.fullmatch(self.start_token)),
            "formal AE process receipt start token is invalid",
        )

    def to_json(self) -> dict[str, Any]:
        return {
            "formalAeApp": self.formal_ae_app,
...
async def _read_bounded_process_stream(
    ...
    value = bytearray()
    while True:
        remaining = PROCESS_OUTPUT_LIMIT + 1 - len(value)
        if remaining <= 0:
            raise DevelopmentSmokeFailure("process inspection output is oversized")
...
async def _run_process_inspection(
    command: tuple[str, ...],
) -> ProcessInspectionResult:
    require(
        bool(command)
        and command[0] == "/bin/ps"
        and (
            command == ("/bin/ps", "-axo", "pid=,lstart=,comm=")
            or (
                len(command) == 5
                and command[1] == "-p"
                and command[2].isdigit()
                and command[3:] == ("-o", "pid=,lstart=,comm=")
            )
        ),
        "process inspection command is not allowed",
    )
    try:
        ...
        require(
            ...
            "process inspection streams are unavailable",
        )
        stdout_bytes, stderr_bytes = await asyncio.wait_for(
            asyncio.gather(
                _read_bounded_process_stream(process.stdout),
                _read_bounded_process_stream(process.stderr),
            ),
            timeout=5,
        )
        returncode = await asyncio.wait_for(process.wait(), timeout=5)
    except (DevelopmentSmokeFailure, OSError, asyncio.TimeoutError) as error:
        if "process" in locals() and process.returncode is None:
            process.kill()
            await process.wait()
        if isinstance(error, DevelopmentSmokeFailure):
            raise
        raise DevelopmentSmokeFailure(
            "formal AE process inspection failed"
        ) from error
    try:
        ...
        raise DevelopmentSmokeFailure(
            "formal AE process inspection returned non-UTF-8 output"
        ) from error
...
def _validate_process_result(result: ProcessInspectionResult) -> None:
    require(
        isinstance(result, ProcessInspectionResult),
        "formal AE process inspection result is invalid",
    )
    require(
        len(result.stdout.encode("utf-8")) <= PROCESS_OUTPUT_LIMIT
        and len(result.stderr.encode("utf-8")) <= PROCESS_OUTPUT_LIMIT,
        "process inspection output is oversized",
    )


def _parse_process_rows(stdout: str) -> list[tuple[int, str, str]]:
    rows: list[tuple[int, str, str]] = []
    for line in stdout.splitlines():
        require(bool(line.strip()), "formal AE process output is malformed")
        match = PROCESS_ROW.fullmatch(line)
        require(match is not None, "formal AE process output is malformed")
        pid = int(match.group(1))
        require(pid > 0, "formal AE process output PID is invalid")
        rows.append((pid, " ".join(match.group(2).split()), match.group(3)))
    return rows


def _formal_ae_executable(formal_ae_app: Path) -> tuple[Path, Path]:
    require(
        formal_ae_app.is_absolute()
        and formal_ae_app.is_dir()
        and not formal_ae_app.is_symlink(),
        "formal AE application is invalid",
    )
    app = formal_ae_app.resolve(strict=True)
    plist_path = app / "Contents/Info.plist"
    require(
        plist_path.is_file() and not plist_path.is_symlink(),
        "formal AE application plist is invalid",
    )
    try:
        plist = plistlib.loads(plist_path.read_bytes())
    except (OSError, plistlib.InvalidFileException) as error:
        raise DevelopmentSmokeFailure(
            "formal AE application plist is unreadable"
        ) from error
    ...
    require(
        ...
        "formal AE executable name is invalid",
    )
    ...
    require(
        ...
        "formal AE executable is invalid",
    )
    return app, executable.resolve(strict=True)


async def capture_formal_ae_process(
    formal_ae_app: Path,
) -> FormalAEProcessReceipt:
    app, executable = _formal_ae_executable(formal_ae_app)
    result = await _run_process_inspection(
    ...
    matches = [
        row
        for row in _parse_process_rows(result.stdout)
        if row[2] == os.fspath(executable)
    ]
    require(
        len(matches) == 1,
        "expected exactly one formal AE process",
    )
    pid, start_token, _path = matches[0]
    return FormalAEProcessReceipt(
        formal_ae_app=os.fspath(app),
...
async def probe_formal_ae_process(
    receipt: FormalAEProcessReceipt,
) -> FormalAEProcessObservation:
    command = (
        "/bin/ps",
        "-p",
        str(receipt.pid),
    ...
    try:
        ...
        if len(rows) != 1 or rows[0][0] != receipt.pid:
            return FormalAEProcessObservation("unavailable", receipt)
    except DevelopmentSmokeFailure:
        return FormalAEProcessObservation("unavailable", receipt)
    _pid, start_token, executable_path = rows[0]
    state = (
        "running"
        if executable_path == receipt.executable_path
        and start_token == receipt.start_token
        else "mismatch"
    )
    return FormalAEProcessObservation(state, receipt)
