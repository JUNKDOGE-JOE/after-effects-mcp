...
test('readCodexCliConfig parses top-level model/model_provider and matching provider section', () => {
  const toml = [
    ...
    'name = "MediaStorm GLM"',
    'base_url = "https://token.mediastorm.studio/v1"',
  ...
  assert.deepEqual(result, {
    ...
    provider: {
      name: 'MediaStorm GLM',
      baseUrl: 'https://token.mediastorm.studio/v1',
...
test('readCodexCliConfig ignores comments and unrelated sections', () => {
  ...
  assert.equal(result.provider.baseUrl, 'https://token.mediastorm.studio/v1');
});
