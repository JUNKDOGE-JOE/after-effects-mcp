...
test('MCP initializes, enforces loopback Origin/Host, and supports session lifecycle', async () => {
    ...
    try {
        ...
        assert.deepEqual(listed.body.result.tools.map(function (tool) { return tool.name; }), ['ae_status', 'ae_exec', 'ae_previewFrame', 'ae_read', 'ae_checkpoint', 'ae_revert', 'ae_validateExpressions']);
