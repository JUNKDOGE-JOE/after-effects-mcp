...
- **previewFrame 在 16 bpc 工程上可用，且不再撞 Claude Code 的 16 MB 单事件上限（#352，PR #362）**——宿主解码 16-bit RGB/RGBA PNG；预览一律回传 8-bit；每次响应受 12 MiB 总量 / 4.5 MiB 单图预算约束，超限自动缩帧，仍装不下则返回路径加缩略图，`path` 永远指向全分辨率文件；进度通知写真实工具名；首次预览时清理其它 session 中 7 天前或超出 300 MB 的临时帧。
- **Tool Library 密钥扫描不再误伤普通脚本（#353，PR #361）**——`var key = prop.keyTime(1)` 这类赋值不再被判为凭证，敏感变量名只在右值像密钥时才命中；legacy skill 目录里读不了或扫描不过的文件改为跳过并记宿主日志，不再让 `ae_toolSearch` 整体失败。
...
- **previewFrame works on 16 bpc projects and stays under Claude Code's 16 MB event cap (#352, PR #362)** — the host decodes 16-bit PNG, always returns 8-bit previews, and fits every response into a 12 MiB total / 4.5 MiB per-image budget by downscaling (path + thumbnail as the last resort); progress notifications name the running tool; stale temp frames are pruned on first use.
- **Tool Library secret scanning no longer rejects ordinary scripts (#353, PR #361)** — sensitive identifiers only trip when the assigned value looks like a secret; unreadable legacy skill files are skipped with a host-log warning instead of failing the whole search.
