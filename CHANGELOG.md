...
- **ExtendScript 传输信封批量 ASCII 转义**——纯 ASCII 结果走零回调快路径，300 KB 的 `/exec` / `ae_exec` 从 5–6 秒降至亚秒级；非 ASCII 改为按连续段交给引擎原生 `escape()`，结构化 JSON 改用数组汇集后一次 `join`，避免逐字符回调与 O(n²) 字符串累加。信封初始化仍以覆盖控制字符、非 ASCII 与代理对的探针自检，不等价或引擎异常即回退原逐字符实现。
...
- **Batched ASCII escaping for the ExtendScript transport envelope** — pure-ASCII results use a zero-callback fast path, cutting 300 KB `/exec` / `ae_exec` calls from 5–6 seconds to sub-second latency. Non-ASCII text is handed to the engine's native `escape()` in contiguous runs, while structured JSON collects parts for one final `join`, avoiding per-character callbacks and O(n²) string accumulation. An initialization probe covering control characters, non-ASCII text, and surrogate pairs still falls back to the character loop on any mismatch or engine error.
