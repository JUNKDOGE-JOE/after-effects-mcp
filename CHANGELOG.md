...
- **ExtendScript 传输信封批量 ASCII 转义**——字符串结果改走纯 ASCII 快路径或正则一次替换，信封初始化时以覆盖控制字符、非 ASCII 与代理对的探针自检，不等价或引擎异常即回退原逐字符实现；300 KB 结果的 `/exec` / `ae_exec` 从 5–6 秒降至亚秒级。
...
- **Batched ASCII escaping for the ExtendScript transport envelope** — string results now use a pure-ASCII fast path or one regexp replacement; an initialization probe covering control characters, non-ASCII text, and surrogate pairs falls back to the character loop on any mismatch or engine error. This cuts 300 KB `/exec` / `ae_exec` results from 5–6 seconds to sub-second latency.
